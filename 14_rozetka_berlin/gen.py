...
def get_price(width):
    if width <= 150:
        return round(7.98 + (width - 30) * 0.13, 2)
    else:
        return round(23.44 + (width - 150) * 0.13, 2)
...
for i in dns.rolet:
    for j in dns.price.keys():
        sku = f'berlin{i[0]}{j}'
        title = f'Рулонная штора Дрезден {i[0]} Miranda {j}x170 см {i[1]}'
        title_ua = f'Рулонна штора Дрезден {i[0]} Miranda {j}x170 см {i[2]}'
        image = i[3]
        size = j
...
for item in hub:
    ...
    my_file.write('<picture>https://raw.githubusercontent.com/relavus1981/rozetka/main/img-len/85ca5c7130208f43e8aea6e35aeb5259.jpg</picture>\n')
    ...
    my_file.write(f'<vendor>Miranda</vendor>\n')
