...
def get_price(width):
    if width < 150:
        return round(8.70 + (width - 30) * 0.15, 2)
    else:
        return round(26.71 + (width - 150) * 0.15, 2)
...
for i in dns.rolet:
    for j in dns.price.keys():
        sku = f'cvety{i[0]}{j}'
        title = f'Рулонная штора цветы  {i[0]} Decoharm {j}x170 см {i[1]}'
        title_ua = f'Рулонна штора квіти  {i[0]} DecoSharm {j}x170 см {i[2]}'
        image = i[3]
        size = j
