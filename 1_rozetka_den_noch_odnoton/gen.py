...
def get_price(width):
    if width < 170:
        return round(9.6 + (width - 30) * 0.16, 2)
    else:
        return round(47.21 + (width - 170) * 0.25, 2)
...
for i in dns.rolet:
    for j in dns.price.keys():
        sku = f'dn{i[0]}{j}'
        title = f'Ролета день ночь Decoharm {j}x160 см {i[1]}'
...
for item in hub:
    ...
    my_file.write(f'<param name="Размеры" paramid="99600">{item.size}x160 см</param>\n')
    ...
    my_file.write(f'<param name="Тип" paramid="95278"><![CDATA[ Ролеты день-ночь, Ролеты тканевые, Рулонные шторы ]]></param>\n')
