...
def get_price(width):
    if width < 150:
        return round(7.58 + (width - 30) * 0.11, 2)
    else:
        return round(21.08 + (width - 150) * 0.11, 2)
...
for i in dns.rolet:
    for j in dns.price.keys():
        sku = f'natura{i[0]}{j}'
        title = f'Рулонная штора натура {i[0]} Decoharm {j}x170 см {i[1]}'
        title_ua = f'Рулонна штора натура {i[0]} DecoSharm {j}x170 см {i[2]}'
        image = i[3]
        size = j
