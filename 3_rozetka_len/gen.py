...
def get_price(width):
    if width < 150:
        return round(7.19 + (width - 30) * 0.10, 2)
    else:
        return round(19.16 + (width - 150) * 0.10, 2)
...
for i in dns.rolet:
    for j in dns.price.keys():
        sku = f'len{i[0]}{j}'
        title = f'Рулонная штора лен {i[0]} Decoharm {j}x170 см {i[1]}'
        title_ua = f'Рулонна штора льон {i[0]} DecoSharm {j}x170 см {i[2]}'
        image = i[3]
        size = j
